...
public interface BookingJpaRepository extends CommonRepository<Booking, Integer> {
    ...
    @Query("SELECT b FROM Booking b where b.bookingUser.id=:userId and b.realEndDate is null")
    Collection<Booking> getActiveBookingsByUserId(@Param("userId") int userId);
